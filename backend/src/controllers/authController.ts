...
export async function register(c: Context) {
    try {
        ...
        const user = await userService.registerWithEmailPassword(email, password, displayName);
...
export async function verifyPhoneOTP(c: Context) {
    try {
        const body = await c.req.json();
        const { phone, otp, name, fullName } = body;
        ...
        // Find or create user
        const user = await userService.findOrCreateUserByPhone(phone, displayName);
